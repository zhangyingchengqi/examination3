...
function showPageInfo(data) {
    ...
    var pageStr = "";
    pageStr = pageStr
            + '<tr><th bordercolor="#7EA6DC">编号</th><th bordercolor="#7EA6DC">考试日期</th><th bordercolor="#7EA6DC">班级</th><th bordercolor="#7EA6DC">试卷名称</th><th bordercolor="#7EA6DC">状态</th><th bordercolor="#7EA6DC">操作</th></tr>';
                ...
                .each(
                        ...
                        function(i, pageinfo) {
                            ...
                            pageStr += '<td align="center">'
                                    + pageinfo.examDate + '</td>';
                            pageStr += '<td align="center">'
                                    + pageinfo.examineeClass + '</td>';
                            pageStr += '<td align="left">' + pageinfo.paperName
                                    + '</td>';
                            var statusStr = "";
                            ...
                            pageStr += '<td align="center">' + statusStr
                                    + '</td>';
                            if (pageinfo.paperStatus == 3) {
                                pageStr += '<td align="center"><a href="#" onclick="javascript:showwrittiongpapergrade(\''
                                        + pageinfo.id
                                        + '\',\''
                                        + pageinfo.paperName
                                        + '\',\''
                                        + pageinfo.examineeClass
                                        + '\',\''
                                        + pageinfo.examSubject
                                        + '\',\''
                                        + pageinfo.examDate
                                        + '\',\''
                                        + pageinfo.scorePercent
                                        + '\',\''
                                        + pageinfo.avgScore
                                        + '\',\''
                                        + pageinfo.maxScore
                                        ...
                                        + pageinfo.minScore
                                        + '\',\''
                                        + pageinfo.questionInfo
                                        + '\')" title="查看已评试卷的评卷结果">查看</a> &nbsp; / &nbsp;<a href="#" onClick="deletePaper(\''
                                        + pageinfo.id
                                        + '\')"title="删除此卷">删除</a></td>';
                            } else if (pageinfo.paperStatus == 1) {
                                pageStr += '<td align="center"><a href="#" onclick="updatePaperStatus('
                                        + pageinfo.id
                                        + ',\'lbl_${rowNumber.index}\');return false";title="开考此卷，也可以在“考试安排”菜单中开考">开考</a><a href="#" onClick="javascript:deletePaper('
                                        + pageinfo.id
                                        + ')" title="删除此卷">删除</a> </td>';
                            } else if (pageinfo.paperStatus == 4) {
                                pageStr += '<td align="center"><a href="/Examination2.0/back/grade/showwritingpapergrade.html" title="查看正在考试的试卷">查看</a>&nbsp; <a href="#" onClick="javascript:deletePaper('
                                        + pageinfo.id
                                        + ')" title="删除此卷">删除</a></td>';
                            } else if (pageinfo.paperStatus == 2) {
                                pageStr += '<td align="center"> <a id="gradeA" href="#" onClick="gradePaper(this,'
                                        + pageinfo.id
                                        + ',\'lbl_${rowNumber.index}\')" title="自动评卷">评卷</a>&nbsp; <a href="#" onClick="javascript:deletePaper('
                                        + pageinfo.id
                                        + ')" title="删除此卷">删除</a></td>';
                            }
                        });
...
// 显示班级考试情况（带图表显示的）
function showwrittiongpapergrade(wpid, paperName, examineeClass, examSubject,
        examDate, scorePercent, avgScore, maxScore, minScore, questionInfo) {
    if (window.localStorage) {
