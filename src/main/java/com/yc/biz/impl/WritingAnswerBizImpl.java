...
public class WritingAnswerBizImpl implements WritingAnswerBiz
{
	 ...
	 */
	@Override
	public int exeUpdateGrade(WritingAnswer writingAnswer)
	{
		WritingAnswer wa=new WritingAnswer();
		wa=(WritingAnswer) this.baseDao.get(wa.getClass(), writingAnswer.getId());
		Float a=writingAnswer.getScore();
		wa.setScore(a);
		baseDao.update(wa);
		return 0;
	}
	...
	public boolean isHandInPaper(String paperId, String examineeName)
	{
		boolean flag=false;
		String[] params=new String[]{paperId,examineeName };
		String sql = "from WritingAnswer wa where wa.id =? and wa.examineeName =? and wa.score is not null";
		List list=this.baseDao.search(sql,params);
		if( list!=null&&list.size()>0 ){
			flag =  true;
		}	
