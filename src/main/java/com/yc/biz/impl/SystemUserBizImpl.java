...
public class SystemUserBizImpl implements SystemUserBiz
{
	...
	{
		String sql = "select s.authorities from SystemUser s where s.userName=? and s.password=?";
    ...
    @Override
	public List<SystemUser> searchTeacherInfo() {
        String hqlString = "from SystemUser";
        List<SystemUser> list=(List<SystemUser>) this.baseDao.search(hqlString);
        if( list!=null&&list.size()>0 ){
            return list;
        }   
        return null;
