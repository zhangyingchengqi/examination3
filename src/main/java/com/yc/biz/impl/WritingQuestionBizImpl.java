...
public class WritingQuestionBizImpl implements WritingQuestionBiz {
	...
	public int deleteManyWritingQuestion(String quesiontIDs) {
		String sql = "delete   from   WritingQuestion where   id   in ("
				+ quesiontIDs + ") ";
		return baseDao.executeHql(sql, null);
	}
...
//	}
	...
	public DataGaidModel searchAllWritingQuestionPage(DataarrayActionModel da) {
		...
		String sort = "wp." + s;
		if (da != null) {
			if (da.getQuestionType() != null && da.getQuestionType() != 3) {
				sql += " and wp.questionType = '" + da.getQuestionType() + "'";
			...
			if (da.getDifficult() != null && da.getDifficult() != 4) {
				sql += " and wp.difficulty = " + da.getDifficult();
			...
			if (da.getTxtQuestion() != null && !"".equals(da.getTxtQuestion())) {
				sql += " and wp.question like '%" + da.getTxtQuestion() + "%'";
				sqlcount += " and wp.question like '%" + da.getTxtQuestion()
						+ "%'";
				// param.put("question", "%"+da.getTxtQuestion()+"%");
			}
		}
		List<WritingQuestion> list = baseDao.findByProperty(sql, param, offset,
				rows, sort, order);
