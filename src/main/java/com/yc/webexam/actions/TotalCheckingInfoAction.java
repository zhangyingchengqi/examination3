...
public class TotalCheckingInfoAction extends BaseAction implements ServletRequestAware,
	...
	//根据查询组合条件统计(教师端)
		private void findCheckingInfoByAll(HttpServletRequest request,HttpServletResponse response) {
		...
		private String readyCheckingHtml(int cid,String uname) {
			//		StringBuffer stf=new StringBuffer("<%@ page language=\"java\" contentType=\"text/html; charset=UTF-8\" import=\"java.util.*\" pageEncoding=\"UTF-8\"%>" +
			//				"<%String path = request.getContextPath();String basePath = request.getScheme() +\"://\"+ request.getServerName() + \":\" + request.getServerPort()+ path + \"/\";%>" +
			//				"<html><head><base href=\"<%=basePath%>\"/><link href=\"/ExamModule/Jsp/js/szindex.css\" type=\"text/css\" rel=\"stylesheet\"></head>" +
			//		"<body><table border=\"1\" cellpadding=\"1\" bordercolor=\"#FFFFFF\" cellspacing=\"1\" >");
			StringBuffer stf=new StringBuffer("<table width=\"1000px\" border=\"1\" cellpadding=\"1\" bordercolor=\"#FFFFFF\" cellspacing=\"1\" >");
			...
			if(uname!=null&&!"".equalsIgnoreCase(uname)&&!"0".equals(uname)){
				...
				if(singleCheckingInfo!=null&&singleCheckingInfo.size()>0){
					stf.append("<tr><th bordercolor=\"#7EA6DC\">日&nbsp;&nbsp;期</th><th bordercolor=\"#7EA6DC\">状&nbsp;&nbsp;态</th><th bordercolor=\"#7EA6DC\">备&nbsp;&nbsp;注</th></tr>");
					for(Temp tp:singleCheckingInfo){
						line.append("<tr align=\"center\" onmouseover=\"this.bgColor='#93BBDF'\" onmouseout=\"this.bgColor='#EDECEB';\" height=\"25\">");
						line.append("<td bordercolor=\"#7EA6DC\">"+tp.getSubname()+"&nbsp;&nbsp;"+tp.getPointidTime()+"</td>");
					...
					stf.append("<tr align=\"center\" height=\"40\"><td colspan=\"3\" style=\"font-weight:bold;font-size:18px;\">");
					if(list!=null&&list.size()>0){
						stf.append(uname+"&nbsp;&nbsp;考勤总数为："+sum+"次&nbsp;&nbsp;&nbsp;&nbsp;");
						for(Temp tp:list){
							if(tp.getSubid()==3||tp.getSubid()==4){
								stf.append(tp.getSubidStr()+"：<span style=\"color:green;font-weight:bold;font-size:18px;\">"+tp.getClassid()+"次</span>&nbsp;&nbsp;&nbsp;&nbsp;");
							}else if(tp.getSubid()==2||tp.getSubid()==5||tp.getSubid()==6){
								stf.append(tp.getSubidStr()+"：<span style=\"color:green;font-weight:bold;font-size:18px;\">"+tp.getClassid()+"次</span>&nbsp;&nbsp;&nbsp;&nbsp;");
							}else{
								stf.append(tp.getSubidStr()+"：<span style=\"font-weight:bold;font-size:18px;\">"+tp.getClassid()+"次</span>&nbsp;&nbsp;&nbsp;&nbsp;");
							}
						}
					}
					stf.append("</td></tr>");
				}else{
					stf.append("<tr><td><span style=\"color:red;font-weight:bold;font-size:18px;\">暂无您要查询的考勤记录</span></td></tr>");
				}
				stf.append("</table>");
			}else{
				//获取考勤学生的姓名
				...
				if(checkingStuInfo!=null&&checkingStuInfo.size()>0){
					...
					for(Temp tp:checkingStuInfo){
						count++;
						stf.append("<th width=\""+(900/checkingStuInfo.size())+"px\" id=\"student"+count+"\" bordercolor=\"#7EA6DC\" onmouseover=\"totalMyCheckingInfo('"+tp.getSname()+"','student"+count+"')\" onmouseout=\"totalMyCheckingout('student"+count+"')\">"+tp.getSname()+"</th>");
				...
				//获取考勤日期
				stf.append("<div id=\"findCheckingResultInfoes\"><table width=\"1000px\" border=\"1\" cellpadding=\"1\" bordercolor=\"#FFFFFF\" cellspacing=\"1\" >");
				...
				if(checkingDateTime!=null&&checkingDateTime.size()>0){
					...
					for(Temp tp:checkingDateTime){
						count++;
						line.append("<tr height=\"25\" align=\"center\"><td width=\"8%\" id=\"stuBydateInfo"+count+"\" bordercolor=\"#7EA6DC\" onmouseover=\"totalDateCheckingInfo('stuBydateInfo"+count+"','"+tp.getPpid()+"')\" onmouseout=\"totalDateCheckingout('stuBydateInfo"+count+"')\">"+tp.getSubname()+" "+tp.getPointidTime()+"</td>");
						if(allInfo!=null&&allInfo.size()>0){
							for(Temp tp1:checkingStuInfo){
								flag=true;
								for(Temp tp2:allInfo){
									count1++;
									if(tp1.getSname().equals(tp2.getSname())&&tp2.getSubname().equals(tp.getSubname())&&tp2.getPointid()==tp.getPointid()){
										flag=false;
										if(tp2.getSubid()==3||tp2.getSubid()==4){
											line.append("<td id=\"stuInfo"+count1+"\" onmouseover=\"checkingInfoByPointOver('stuInfo"+count1+"','"+tp2.getPpid()+"','"+tp1.getSname()+"')\" onmouseout=\"checkingInfoByPointOut('stuInfo"+count1+"')\" bordercolor=\"#7EA6DC\" style='color:green'>"+tp2.getSubidStr()+"</td>");
										}else if(tp2.getSubid()==2||tp2.getSubid()==5||tp2.getSubid()==6){
											line.append("<td id=\"stuInfo"+count1+"\" onmouseover=\"checkingInfoByPointOver('stuInfo"+count1+"','"+tp2.getPpid()+"','"+tp1.getSname()+"')\" onmouseout=\"checkingInfoByPointOut('stuInfo"+count1+"')\" bordercolor=\"#7EA6DC\" style='color:red'>"+tp2.getSubidStr()+"</td>");
										}else{
											line.append("<td id=\"stuInfo"+count1+"\" onmouseover=\"checkingInfoByPointOver('stuInfo"+count1+"','"+tp2.getPpid()+"','"+tp1.getSname()+"')\" onmouseout=\"checkingInfoByPointOut('stuInfo"+count1+"')\" bordercolor=\"#7EA6DC\">"+tp2.getSubidStr()+"</td>");
										}
										break;
									}
								}
					...
					//				else{
					//					stf.append("<br /><br /><font color=\"red\"><b>警告：</b></font><br /><font size=\"4\">无</font>");
					//				}

				}else{
					stf.append("<tr><td><span style=\"color:red;font-weight:bold;font-size:18px;\">暂无您要查询的考勤记录</span></td></tr></table>");
				}
			}
