...
{	private static final Logger logger = Logger.getLogger(LoginAction.class);
	...
	public void subject(){
		...
		try {
			if(semester!=null&&editionId!=null){
				subjects=subjectBiz.findSubject(semester, editionId);
			}else{
				subjects=subjectBiz.getSubjects();
			}
			for(int i=0;i<subjects.size();i++){
				subjects.get(i).setChapters(null);
				subjects.get(i).setEdition(null);
			}
	...
	public void examClass(){
		...
		try {
			if(semester==null){
				examineeClass=examineeClassBiz.findAllExamineeClass();
			}else{
				examineeClass=examineeClassBiz.findExamineeClassBySemester(semester);
			}
				for(int i=0;i<examineeClass.size();i++){
				examineeClass.get(i).setaDailyTalks(null);
				examineeClass.get(i).setCheckings(null);
				examineeClass.get(i).setPointPapers(null);
			}
	...
	public void initData() throws UnsupportedEncodingException
	{
		...
		if (requestState.equals("subject"))
		{
			...
			// String version = ValueUtil.formatRequestStr(request.getParameter("version"));
