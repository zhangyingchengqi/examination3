...
public class PointAnswerAction extends BaseAction {
	...
	private static final Logger logger = Logger
			.getLogger(PointAnswerAction.class);
					...
					if(list!=null&&list.size()>0){
						...
						if(result>=0){
							for(PointAnswer pa:list){
								int flag=1;
								String Answer=pointAnswerBiz.findPointAnswer(pa.getName(), pa.getCladdid()+"", pa.getPointPaper().getPid()+"");
								...
								while(flag>0){
									...
									int grade=Integer.parseInt(answer.substring(answer.indexOf("-", 0)+1,answer.indexOf(",", 0)));
									if(pointid>0){
