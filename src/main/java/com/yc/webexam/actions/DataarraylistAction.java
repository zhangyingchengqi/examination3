...
public class DataarraylistAction extends BaseAction {
	...
	public void findAllExamineeClassAndId(){
		String jsonStr="";
		try {
		//List<String> list=examineeClassBiz.searchAllExamineeClassNameAndId();
	...
	public void showExamineeNames(){
		...
		try {
			...
			jsonStr=JSON.toJSONString(new JsonProtocol(0,null, list),SerializerFeature.DisableCircularReferenceDetect);
			
		} catch (Exception e) {
			jsonStr=JSON.toJSONString(new JsonProtocol(1,"查询失败！", null),SerializerFeature.DisableCircularReferenceDetect);
			logger.error(e);
		}finally{
			try {
				JsonUtil.jsonOut(jsonStr);
