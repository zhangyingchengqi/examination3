...
function showClassName(semester,cid) {
	$.post("/Examination2.0/examineeclass_showClassList.action", {
		...
		for ( var i = 0; i < obj.length; i++) {
			...
			if(obj[i].id==cid){
				$("#examineeClass option[value='"+cid+"']").attr("selected", "selected");
			}
		}
...
function showAdailyTalk() {
	...
	var status = 1;
	$.getJSON("/Examination2.0/checking_showHistory.action",{status:status,className:className,name:name},function(data) {
		if(data.responseCode==0){
			...
			for(var i=0;i<data.obj.length;i++){
				...
				str+="<td width=12% align='center'>"+data.obj[i].name+"</td>";
				str+="<td width=45%>"+data.obj[i].knowledge+"</td>";
				str+="<td align='center' width='15%'>"+data.obj[i].speakdate.substr(0,10)+"</td>";
				if(data.obj[i].status==2){
					str+="<td align='center' width='17%'><font style='color:red;font-size:12px'>[未传]</font><a href='javascript:delKnowledgeInfo("+data.obj[i].id+")'>[删除]</a><a style='font-size:12px' title='"+data.obj[i].assessment+"'>[详细]</a></td>"
				}else	if(data.obj[i].status==3){
					str+="<td align='center' width='17%'><font style='color:blue;font-size:12px'>[下载]</font><a href='javascript:delKnowledgeInfo("+data.obj[i].id+")'>[删除]</a><a style='font-size:12px' title='"+data.obj[i].assessment+"'>[详细]</a></td>"
				}
