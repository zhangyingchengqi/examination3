...
//	alert(cid);
	...
	/*if(index>count){
		alert("学生记录数不正确，请全部考完后再提交修改");
		return;
	}*/
	$.post("/Examination2.0/checking_subchangeChecking.action",
			{id:id,checkDate:date,checkTime:time,checkRemark:remark,checkResult:resultInfo},function(json){
...
function showcheckingresultinfos(checkId){
	if(checkId==null){
		$("checkClassInfo table").append("<tr height='30'><td align='center' colspan='4'><span class='fontColor' style='color:red;font-weight:bold;font-size:20px;'>暂无考勤记录</span></td></tr>")
	}else{
		$.post("/Examination2.0/checking_showcheckingresultinfos.action",{checkId:checkId},function(json){
			var obj=$.parseJSON(json);
			if(obj.responseCode==0){
				...
				for(var i=0;i<crs.length;i++){
					...
					var stuName=stu[0];
					var stuStatus=stu[1];
					var status=stuStatus.split("-")[0];
					var remark=stuStatus.split("-")[1];
					$("#checkClassInfo table").append("<tr height='23' id='student"+i+"' bgcolor='#EDECEB' onmouseover=this.bgColor='#93BBDF'; onmouseout=this.bgColor='#EDECEB'; align='left'>"
				+"<td align='center' width='8%'>"+(i+1)+"</td>"
				+"<td align='center' width='10%' id='stuName"+(i+1)+"' >"+stuName+"</td>"
				+"<td width='40%' align='center' id='radio"+i+"'>"
				+"<input type='radio' name='status"+i+"'  id='status1' value='"+stuName+",1' />已到&nbsp;&nbsp;&nbsp;"
				+"<input type='radio' name='status"+i+"'  id='status2' value='"+stuName+",2' />迟到&nbsp;&nbsp;&nbsp;"
				+"<input type='radio' name='status"+i+"'  id='status3' value='"+stuName+",3' />病假&nbsp;&nbsp;&nbsp;"
				+"<input type='radio' name='status"+i+"'  id='status4' value='"+stuName+",4' />请假&nbsp;&nbsp;&nbsp;"
				+"<input type='radio' name='status"+i+"'  id='status5' value='"+stuName+",5' />旷课&nbsp;&nbsp;&nbsp;"
				+"<input type='radio' name='status"+i+"'  id='status6' value='"+stuName+",6' />早退&nbsp;&nbsp;&nbsp;"
				+"</td>"
				+"<td width='38%'><input type='text' size='56' id='remarkInfo"+(i+1)+"' value='"+remark+"' /></td>"
				+"</tr>")
				
				$("#radio"+i+"  #status"+status).attr("checked",true);
//					alert(stuName+ "\t"+  status +"\t"+ remark);
